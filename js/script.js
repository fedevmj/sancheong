window.onload = function () {
  ...
  for (let i = 0; i < sw_notice_total; i++) {
    ...
    <div class=swiper-slide>
      <a href="${temp_data.link}" class="sw-notice-link">
        <img src="images/${temp_data.imgurl}" alt="${temp_data.alt}">
      </a>
    </div>`;
  }
  ...
  $.each(gnb_a, function (index, item) {
    $(this).mouseenter(function () {
      // 이미 선택된 index의 메뉴는 작동시키지 않는다.(중복 작동 X)
      // 처음 시작할 때 header_index == 10000이어서 mouseover시 index와 다름.
      // 그래서 메뉴가 작동됨. 그때 header_index 값을 index로 교체.
      // 다음에 mouseover가 되더라도 값이 같다면 재실행하지 않도록 막아둠.
